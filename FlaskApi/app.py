...
def on_connectHum(client, userdata, msg, rc):

    print("connected with result code " + str(rc))
    client.subscribe("topic/humid")
...
def on_messageT(client, userdata, msg):
...
def on_messageHum(client, userdata, msg):
    json_body = json.loads(msg.payload.decode())
    keys = json_body.keys()
    payload = {
        "included_segments": ["Subscribed Users"],
        "contents":{
            "en": "Humedad: " + str(json_body["humedad"])
        },
        "name": "NOTIFICATION TESTS",
        "app_id": "ccc3e0ea-e6e8-44d1-93db-4287eb8507a8"
    }
...
def on_messageH(client, userdata, msg):
    json_body = json.loads(msg.payload.decode())
    keys = json_body.keys()
    payload = {
        "included_segments": ["Subscribed Users"],
        "contents":{
            "en": "Humo: " + str(json_body["humo"]) 
        },
        "name": "NOTIFICATION TESTS",
        "app_id": "ccc3e0ea-e6e8-44d1-93db-4287eb8507a8"
    }
...
def on_messageM(client, userdata, msg):
    ...
    payload = {
        "included_segments": ["Subscribed Users"],
        "contents":{
            "en": "Movimiento: " + str(json_body["mov"])
        },
        "name": "NOTIFICATION TESTS",
        "app_id": "ccc3e0ea-e6e8-44d1-93db-4287eb8507a8"
    }
...
@app.route('/postHumedad',methods = ['POST'])
def postHum():
    ...
    client.on_message = on_messageHum
